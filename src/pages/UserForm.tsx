      ...
      <div className="w-full max-w-3xl mx-auto glass-morphism rounded-xl p-8 md:p-10 animate-fade-in">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gradient mb-2 font-sans">
            {userType === 'borrower' ? 'Apply for a Loan' : 'Financial Institution Registration'}
          </h1>
          ...
          <div className="flex justify-between items-center">
            <div 
              className={`flex-1 text-center pb-2 border-b-2 ${
                currentSection === 'personal' ? 'border-fintech-500 text-white' : 'border-white/20 text-white/50'
              } transition-colors`}
            >
              Personal Info
            </div>
            <div 
              className={`flex-1 text-center pb-2 border-b-2 ${
                currentSection === 'documents' ? 'border-fintech-500 text-white' : 'border-white/20 text-white/50'
              } transition-colors`}
            >
              Documents
            </div>
            <div 
              className={`flex-1 text-center pb-2 border-b-2 ${
                currentSection === 'loan' ? 'border-fintech-500 text-white' : 'border-white/20 text-white/50'
              } transition-colors`}
            >
              Loan Details
            </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {currentSection === 'documents' && (
            <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      {formData.bankStatement ? (
                        <>
                          ...
                          <p className="text-sm text-fintech-400">{formData.bankStatement.name}</p>
                        </>
                      ) : (
                        <>
                          <Upload className="w-8 h-8 mb-2 text-white/70" />
                          <p className="mb-1 text-sm text-white/70">Click to upload bank statement</p>
                          <p className="text-xs text-white/50">PDF up to 10MB</p>
                        </>
                      )}
                    </div>
                    <Input 
              ...
              <div>
                <Label htmlFor="aisDocument">Annual Information Statement (AIS) (PDF)</Label>
                <div className="mt-1 flex items-center justify-center w-full">
                  <label 
                    htmlFor="aisDocument" 
                    className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer border-white/10 bg-white/5 hover:bg-white/10 transition-colors"
                  >
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      {formData.aisDocument ? (
                        <>
                          <FileText className="w-8 h-8 mb-2 text-fintech-400" />
                          <p className="text-sm text-fintech-400">{formData.aisDocument.name}</p>
                        </>
                      ) : (
                        <>
                          <Upload className="w-8 h-8 mb-2 text-white/70" />
                          <p className="mb-1 text-sm text-white/70">Click to upload AIS document</p>
                          <p className="text-xs text-white/50">PDF up to 10MB</p>
                        </>
                      )}
                    </div>
